...
$precio = $_POST['precio'];
setcookie('precio',$precio, time()+60);

$idhab = $_POST['hab'];
setcookie('idhab', $idhab, time()+60);

$llegada = $_POST['desde'];
setcookie('llegada', $llegada, time()+60);

$salida = $_POST['hasta'];
setcookie('salida', $salida, time()+60);

$dias = $_POST['dias'];
setcookie('dias',$dias, time()+60);
...
$tels = $_POST['tels'];
setcookie('tels', $tels, time()+60);

$tipo = $_POST['tipo'];
setcookie('tipo', $tipo, time()+60);
...
if(! isset($token)) {
		...
		/* The servername and serverport tells PayPal where the buyer
		 ...
		 /* Construct the parameter string that describes the PayPal payment
           ...
           $nvpstr="&ADDRESSOVERRIDE=1$shiptoAddress&L_NAME1=".$L_NAME1."&L_AMT1=".$L_AMT1."&L_QTY1=".$L_QTY1."&MAXAMT=".(string)$maxamt."&AMT=".(string)$amt."&ITEMAMT=".(string)$itemamt."&CALLBACKTIMEOUT=4&L_SHIPPINGOPTIONAMOUNT1=8.00&L_SHIPPINGOPTIONlABEL1=UPS Next Day Air&L_SHIPPINGOPTIONNAME1=UPS Air&L_SHIPPINGOPTIONISDEFAULT1=true&L_SHIPPINGOPTIONAMOUNT0=3.00&L_SHIPPINGOPTIONLABEL0=UPS Ground 7 Days&L_SHIPPINGOPTIONNAME0=Ground&L_SHIPPINGOPTIONISDEFAULT0=false&INSURANCEAMT=1.00&INSURANCEOPTIONOFFERED=true&CALLBACK=https://www.ppcallback.com/callback.pl&SHIPPINGAMT=8.00&SHIPDISCAMT=-3.00&TAXAMT=2.00&L_NUMBER0=&L_DESC0=&L_NUMBER1=".$habitacion."&L_DESC1=&L_ITEMWEIGHTVALUE1=0.5&L_ITEMWEIGHTUNIT1=lbs&ReturnUrl=".$returnURL."&CANCELURL=".$cancelURL ."&CURRENCYCODE=".$currencyCodeType."&PAYMENTACTION=".$paymentType;
...
{
    // Logo
    $this->Image('../images/logopdf.png',10,10,50);
    // Arial bold 15
...
$pdf->Cell(0,20,utf8_decode('Aukute Inn Hotel'),0,0,'C');
    $pdf->Ln();
$pdf->SetFont('Helvetica', '',12);
$pdf->Cell(0,10,utf8_decode('Blv. Pedro Baznariz #457, Puerto Vallarta, Jalisco'),0,0,'C');
    $pdf->Ln();
$pdf->Cell(0,10,utf8_decode('Tels: 5742798, 5724632'),0,0,'C');
    $pdf->Ln();
$pdf->Cell(0,10,utf8_decode('e-mail: reservaciones@aukute.com'),0,0,'C');
    $pdf->Ln();
